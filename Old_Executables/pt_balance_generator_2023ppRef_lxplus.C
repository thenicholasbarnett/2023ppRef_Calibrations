...
void pt_balance_generator_2023ppRef_lxplus()
{
    ...
    ifstream myfile("filenames/filenames_MC_ppRef_2023_6s.txt");

    // filename or q is the name of the fi file being opened and analyzed
    string filename;
    while (getline(myfile, filename)){
        // i3 is the number of file we are on
        i3+=1;
        TString q = filename;
        cout<<endl<<"file number "<<i3<<" from the list is open"<<endl<<"the file name is "<<q<<endl;
    ...
    TFile *f1 = new TFile("pt_balance_MC_ppRef_2023_6s.root", "recreate");
    f1->cd();
